...
export default function ProductDetailPage() {
  ...
  const trustItems = [
    ...
    { icon: <PhoneIcon sx={{ fontSize: 16 }} />, label: '+91 XXXXX XXXXX' },
  ];
  ...
  return (
    <Box sx={{ bgcolor: '#F8FBFF', minHeight: '100vh', pb: { xs: 10, md: 6 } }}>
      <Container maxWidth="xl" sx={{ py: 4 }}>
        {/* Breadcrumbs */}
        <Breadcrumbs sx={{ mb: 3 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }} component={Link} to="/" style={{ textDecoration: 'none', color: 'inherit' }}>
            <HomeIcon sx={{ fontSize: 16 }} />
            <Typography variant="caption">Home</Typography>
          </Box>
          <Typography variant="caption" component={Link} to="/products" style={{ textDecoration: 'none', color: 'inherit' }}>Products</Typography>
          <Typography variant="caption" color="text.primary">{product.category}</Typography>
        </Breadcrumbs>

        {/* Main card — no internal scroll, full layout */}
        <Box
          sx={{
            bgcolor: '#fff',
            borderRadius: 3,
        ...
        >
          ...
          <Box sx={{ p: { xs: 3, md: 4 }, display: 'flex', flexDirection: 'column', gap: 0 }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1.5 }}>
              ...
              <Typography variant="caption" color="text.secondary">{product.rating} ({product.reviews} reviews)</Typography>
            </Box>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75, mb: 2 }}>
              ...
              <Typography variant="caption" color="primary.main" fontWeight={500}>Available in Multiple Colors</Typography>
            </Box>
            ...
            {addedToCart && <Alert severity="success" sx={{ mb: 1.5, borderRadius: 2, py: 0.5 }}>Added to cart!</Alert>}
            ...
            <Box sx={{ display: 'flex', gap: 1.5, mb: 1.5, flexWrap: 'wrap' }}>
              ...
              <Button
                ...
                component="a"
                href={`https://wa.me/91XXXXXXXXXX?text=${encodeURIComponent(`Hi, I'm interested in ${product.name}`)}`}
                target="_blank"
            ...
            <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1 }}>
              {trustItems.map(item => (
                <Box key={item.label} sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
                  ...
                  <Typography variant="caption" fontWeight={500} color="text.secondary">{item.label}</Typography>
                </Box>
              ))}
            </Box>
          </Box>
      ...
      {/* <EnquiryModal open={enquiryOpen} onClose={() => setEnquiryOpen(false)} defaultSubject="Product Enquiry" /> */}
    </Box>
  );
}